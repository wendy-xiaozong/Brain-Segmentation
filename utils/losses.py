...
def get_loss(y_true, y_pred):
    ...
    loss_1 = tf.reduce_sum(loss_1, axis=axes, keepdims=True) / nonzero
    loss_1 = tf.reduce_mean(loss_1)
    loss_2 = tf.compat.v1.losses.log_loss(labels=y_true, predictions=y_pred, weights=1 - y_true)
    nonzero = tf.reduce_sum(1 - y_true, axis=axes, keepdims=True) + 1e-9
    loss_2 = tf.reduce_sum(loss_2, axis=axes, keepdims=True) / nonzero
