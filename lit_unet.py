...
class Lightning_Unet(pl.LightningModule):
    def __init__(self, hparams):
        ...
        if not COMPUTECANADA:
            ...
            self.training_subjects = self.subjects[:1]
            self.validation_subjects = self.subjects[:1]
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        train_imageDataset = torchio.ImagesDataset(self.training_subjects)
    ...
    def validation_step(self, batch, batch_id):
        ...
        output_tensor, target_tensor = self.compute_from_aggregating(input, target, False)
        ...
        loss = diceloss.forward(input=output_tensor, target=target_tensor.unsqueeze(dim=1))
        output_tensor = output_tensor.type_as(input)
        target_tensor = target_tensor.type_as(input)
        dice, iou, sensitivity, specificity = get_score(output_tensor, target_tensor)
    ...
    def validation_epoch_end(self, outputs):
        ...
        output_tensor = output_tensor.type_as(outputs[0]['val_step_dice'])
        target_tensor = target_tensor.type_as(outputs[0]['val_step_dice'])
        dice, iou, sensitivity, specificity = get_score(pred=output_tensor, target=target_tensor)
        log_all_info(self,
                     img,
                     target_tensor,
                     output_tensor,
